...
class S2N5kDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        DATA_DIR = os.path.join(base_image_dir, "Nutrition5k", 'metadata')
        if is_val:
            with open(os.path.join(DATA_DIR, 'S2_test_data_n5k_yyh.json')) as f:
                data = json.load(f)
        else:
            with open(os.path.join(DATA_DIR, 'S2_train_data_n5k_yyh.json')) as f:
                data = json.load(f)
        # print(len(data))
        self.data = data
        self.is_val = is_val
