...
def get_vqa_from_hf(task, n = 1, k = 0):
    if task == 1:
...
def main(task, qa_type, model_path, fp16, multi_gpu, chunk_n, chunk_id, limit=np.inf):
    kb_data = get_kb_from_hf()
    url_jpg_map = get_url_jpg_map(kb_data)
    vqa_data = get_vqa_from_hf(task, chunk_n, chunk_id)
...
if __name__ == "__main__":
    ...
    result = main(args.task, args.type, args.model_path, args.fp16, args.multi_gpu, args.chunk_num, args.chunk_id-1)
    export_result(
        result,
