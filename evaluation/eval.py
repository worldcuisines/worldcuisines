...
def get_vqa_from_hf(task):
    if task == 1:
        ...
        raise ValueError("Invalid task number. Please choose from 1 (dish name) or 2 (region)")
...
def load_model_processor(model_path, fp16=True, multi_gpu=False): # TODO: Make this versatile for other model
    ...
    if not multi_gpu:
        ...
        model = LlavaNextForConditionalGeneration.from_pretrained(model_path, torch_dtype=torch.float16, low_cpu_mem_usage=True, device_map='auto') 
...
def eval_instance(model, processor, image_file, query):
    ...
    out = out_with_template[out_with_template.index("ASSISTANT: ") + len("ASSISTANT: ") :]

    return out

def main(task, type, model_path, fp16, multi_gpu, limit=10): # TODO: Change the limit
    ...
    for _,row in tqdm(vqa_data.iterrows(), total=len(vqa_data)):
        image_file = url_jpg_map[row['image_url']] # TODO: MAKE SURE NO NEED IMAGE.OPEN
        query = row['multi_choice_prompt'] if type == 'mc' else row['open_ended_prompt']
        response = eval_instance(model, processor, image_file, query)

        res[row['qa_id']] = response
        count += 1
        if count == limit:
            break
...
def export_result(result: dict, path: str):
    with open(path, 'w') as outfile:
        for id_, prediction in result.items():
            json.dump({"id": id_, "prediction": prediction}, outfile)
            outfile.write('\n')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--type', type=str, default='mc', help='Type of question to evaluate')
    parser.add_argument('--model_path', type=str, default="llava-hf/llava-v1.6-vicuna-7b-hf",
    ...
    export_result(result, f"task{args.task}_{args.type}_pred.json")